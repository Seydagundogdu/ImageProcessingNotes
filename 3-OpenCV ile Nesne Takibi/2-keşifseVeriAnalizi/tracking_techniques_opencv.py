# -*- coding: utf-8 -*-
"""
Created on Fri Mar 11 17:59:13 2022

@author: acer
"""
"""
TAKİP ALGORİTMALARI (BOOSTING)
...
# genel parametreler
initBB = None # seçmiş olduğumuz nesneyi depolayacağımız box
fps = 25 # frame per second
frame_number = [] # frame numarasını depolamak için dizi
f = 0
success_track_frame_success = 0
track_list = []
start_time = time.time() # sayaç başlar (zaman)

while True:
    
    time.sleep(0.01) #videoyu yavaşlat
    
    ret, frame = cap.read() #videoyu oku
    
    if ret:
        
        frame = cv2.resize(frame, dsize = (960,540)) # frame boyutları
        (H, W) = frame.shape[:2] # yükseklik ve genişlik tuple'ı (ilk 2 index alınır. 3. index channel'dır)
        
        # gt
        car_gt = gt[gt.frame_no == f]
        if len(car_gt) != 0:
            x = car_gt.x.values[0] # seriyi array'e çevirip 0. indexi alırız
            y = car_gt.y.values[0]
            w = car_gt.w.values[0]
            h = car_gt.h.values[0]
        ...
        if key == ord("t"): # t'ye basınca video durur space'e basınca devam eder
